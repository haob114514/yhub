...
function iData.value49(secondaryFlag)
	...
	while true do
		...
		local label = AreaId and iData.value13.areaLabel[AreaId] or "未知"
	...
	while iData.value17() do
		if iData.value14.AutoHop then
			if iData.value14.HopCount >= iData.value14.MaxHops then
				...
				iData.value18("Server Hop", "已达到服务器跳转上限。")
			else
...
iData.value10.filterValues = {
	"普通", "罕见", "稀有", "史诗", "传说", "神话", "宇宙", "神秘", "神圣", "永恒"
}
iData.value10.rarityDisplayToInternal = {
	["普通"] = "Common", ["罕见"] = "Uncommon", ["稀有"] = "Rare", ["史诗"] = "Epic",
	["传说"] = "Legendary", ["神话"] = "Mythic", ["宇宙"] = "Cosmic", ["神秘"] = "Secret",
	["神圣"] = "Divine", ["永恒"] = "Eternal"
}
iData.value10.rarityInternalToDisplay = {}
for display, internal in pairs(iData.value10.rarityDisplayToInternal) do
	iData.value10.rarityInternalToDisplay[internal] = display
end
	...
	while iData.value17() do
		...
		if iData.value119.AutoSteal and iData.value23() and (iData.value132() and not iData.value126) then
			...
			local success, result = pcall(function()
				...
				if not flag then
					iData.value18("Auto Steal Free", "尚未设置基地！请走到你的基地，然后点击“将此处设为基地”。")
...
function iData.value137(autoSteal)
	...
	if autoSteal then
		...
		if not iData.value119.ManualBase then
			iData.value18("Auto Steal Free", "提示：请先走到你的基地并点击“将此处设为基地”！")
		end
	else
		iData.value136()
	end
...
iData.value12.TabStealFree:Toggle({
	Title = "启用反作弊绕过",
	Desc = "替换角色 Humanoid，让 WalkSpeed 限制器作用于失效对象",
	Value = false,
	Callback = function(enableAntiCheatBypassCondition)
		...
		if enableAntiCheatBypassCondition then
			task.spawn(function()
				...
				iData.value18("Anti-Cheat", not humReady and "尚未准备好。" or "绕过功能已启用。")
			end)

			return
		end

		iData.value18("Anti-Cheat", "绕过功能已关闭。")
	end,
})
iData.value12.TabStealFree:Toggle({
	Title = "虚空脱困",
	Desc = "绕过功能导致卡在跑步机上时，将你弹出脱困",
	Value = false,
...
iData.value12.TabStealFree:Toggle({
	Title = "启用免费自动偷蛋",
	Desc = "检测你拿到鸡蛋（出现 RUN!! + Drop）→ 自动步行传送回基地",
	Value = false,
...
Dropdown(secondaryTabStealFree, {
	...
	Callback = function(eggRarityFilterData)
		...
		local internalRarity = {}
		for _, v in ipairs(rarityFilter) do
			internalRarity[#internalRarity + 1] = iData.value10.rarityDisplayToInternal[v] or v
		end
		iData.value119.RarityFilter = internalRarity
		iData.value131()
	end,
})
	...
	parent.MouseButton1Click:Connect(function()
		...
		if autoSteal then
			...
			task.spawn(function()
				...
				iData.value18("Auto Steal", "正在以 " .. iData.value10.TRAVEL_SPEED .. " studs 的速度运行地面路线。")
			end)
		else
			iData.value11.travelToken = iData.value11.travelToken + 1
			iData.value11.travelling = false
			iData.value51()
			iData.value18("Auto Steal", "已停止。")
		end
...
TabSteal:Dropdown({
	Title = "农场方式",
	Desc = "速度模式贴地移动；TP 行走模式通过跳跃点移动",
	Values = {
		"速度",
		"TP 行走",
	},
	Value = (FarmMethod == "Speed" and "速度") or (FarmMethod == "TP Walk" and "TP 行走") or FarmMethod,
	Callback = function(farmMethodArgument)
		local farmMethod = iData.value118(farmMethodArgument)[1]
		farmMethod = (farmMethod == "速度" and "Speed") or (farmMethod == "TP 行走" and "TP Walk") or farmMethod
...
iData.value12.TabSteal:Toggle({
	Title = "神秘蛋优先",
	Desc = "放下当前鸡蛋，并立即锁定出现的神秘、神圣或永恒鸡蛋",
	Value = true,
...
iData.value41.AutoHop = iData.value12.TabHop:Toggle({
	Title = "启用自动服务器跳转",
	Desc = "不断切换公共服务器，直到达到跳转次数上限",
	Value = false,
	Callback = function(autoHop)
		iData.value14.AutoHop = autoHop

		if autoHop then
			iData.value18("Server Hop", iData.value14.HopCount .. " / " .. iData.value14.MaxHops .. " 次跳转已使用。")
		end
	end,
})
...
iData.value12.TabHop:Button({
	...
	Callback = function()
		task.spawn(function()
			iData.value18("Server Hop", not iData.value115() and "没有可用服务器响应。" or "正在传送。")
		end)
	end,
})
iData.value12.TabHop:Button({
	...
	Callback = function()
		task.spawn(function()
			...
			recallToSpawnCallback(
				"Recall",
				not (if vector then updateGoToTreadmill(vector, 8) else false) and "未找到出生点。"
					or "已返回出生点。"
			)
		end)
	end,
...
iData.value12.TabHatch:Button({
	...
	Callback = function()
		task.spawn(function()
			iData.value18("Hatch", "已孵化 " .. hatchAll(true) .. " 个鸡蛋。")
		end)
	end,
})
iData.value41.EggESP = iData.value12.TabHatch:Toggle({
	Title = "启用鸡蛋透视",
	Desc = "标记地图上的所有场地鸡蛋并显示稀有度，全地图可见",
	Value = false,
...
iData.value41.FpsBoost = iData.value12.TabHatch:Toggle({
	...
	Callback = function(fpsBoost)
		...
		task.spawn(function()
			if fpsBoost then
				iData.value99()
				iData.value18("FPS Boost", "特效已关闭，宠物已隐藏。")

				return
			end

			iData.value100()
			iData.value18("FPS Boost", "已恢复全部设置。")
		end)
	end,
})
...
iData.value12.TabSell:Button({
	...
	Callback = function()
		iData.value18(
			"Pets",
			not iData.value27(iData.value25.WearBest) and "服务器拒绝了请求。" or "已装备最佳宠物。"
		)
	end,
})
iData.value12.TabSell:Button({
...
iData.value12.TabClaim:Button({
	...
	Callback = function()
		task.spawn(function()
			iData.value18("Claim", "已执行 " .. iData.value103(true) .. " 次领取。")
		end)
	end,
})
iData.value12.TabClaim:Toggle({
	Title = "启用自动升级",
	Desc = "提升基地等级并购买范围内可用的升级",
	Value = false,
...
iData.value12.TabClaim:Button({
	...
	Callback = function()
		task.spawn(function()
			iData.value18("Upgrade", "已执行 " .. iData.value104(true) .. " 次升级。")
		end)
	end,
})
...
iData.value12.TabTread:Toggle({
	...
	Callback = function(autoTreadmill)
		...
		if autoTreadmill then
			iData.value18("Treadmill", "此服务器没有找到跑步机。")
		end
	end,
})
iData.value12.TabTread:Toggle({
...
iData.value12.TabTread:Button({
	...
	Callback = function()
		task.spawn(function()
			...
			iData.value18("Treadmill", not goToTreadmillResult and "无法到达目标。" or "已到达。")
		end)
	end,
})
...
iData.value12.TabConfig:Button({
	Title = "掉入虚空",
	Desc = "在虚空中等待 5 秒并等待重生",
	Callback = function()
		task.spawn(function()
			if not iData.value70() then
				iData.value18("Drop to Void", "没有等到重生。", 6)
			end
		end)
	end,
})
iData.value12.TabConfig:Button({
	...
	Callback = function()
		...
		iData.value18("Remotes", #value28Result .. " 个端点已复制。")
	end,
})
iData.value12.TabConfig:Button({
	Title = "复制基地数据",
	Desc = "将实时基地数据和放置结果复制到剪贴板",
	Callback = function()
		task.spawn(function()
			...
			repeat
				if copyPlotDumpFlag or not nameOption then
					...
					iData.value18("Plot Dump", "已复制，同时保存为 ShardPlot.txt。", 6)
...
iData.value12.TabConfig:Button({
	...
	Callback = function()
		task.spawn(function()
			...
			iData.value18("Place Source", "已复制，并保存为 ShardPlaceSrc.txt 及各模块文件。", 6)
		end)
	end,
})
iData.value12.TabConfig:Button({
	Title = "重置跳转次数",
	Desc = "清除已保存的跳转次数，让最大跳转次数重新开始",
	Callback = function()
		iData.value14.HopCount = 0
		iData.value113(0)
		iData.value18("Server Hop", "跳转次数已重置。")
	end,
})
iData.value12.TabConfig:Button({
	Title = "重新加入服务器",
...
function iData.value146(text)
	...
	if not cloneFlag then
		iData.value18("Visual Pets", "找不到名为： " .. text, 4)
	...
	iData.value18("Visual Pets", "已生成 " .. text .. "（仅客户端）。", 3)
end
...
iData.value12.TabVisualPet:Button({
	...
	Callback = function()
		...
		if #value144Result > 0 then
			...
			iData.value18("Visual Pets", #value144Result .. " 个宠物。", 3)

			return
		end

		iData.value18("Visual Pets", "暂时没有找到宠物模型——请等待游戏加载完成后再试。", 4)
	end,
})
iData.value12.TabVisualPet:Input({
	Title = "自定义名称",
	Desc = "如果列表中没有该宠物，可手动输入名称",
	Placeholder = "例如：Dragon",
...
Button(secondaryTabVisualPet, {
	Title = "生成宠物",
	Desc = "在角色周围克隆所选或输入的宠物",
	Color = color,
	Callback = function()
		if iData.value142.inputName == "" then
			iData.value18("Visual Pets", "请从列表选择宠物，或输入自定义名称。", 4)
...
alternateTabVisualPet:Button({
	...
	Callback = function()
		iData.value147()
		iData.value18("Visual Pets", "所有可视化宠物已移除。", 3)
	end,
})
...
if not iData.value24 then
	iData.value18("Shard 集线器", "Packages.Networking is missing, remote features are offline.", 8)

	return
end
iData.value18("Shard 集线器", "Loaded. " .. #iData.value13.areaOrder .. " areas, " .. #iData.value28() .. " endpoints.", 5)